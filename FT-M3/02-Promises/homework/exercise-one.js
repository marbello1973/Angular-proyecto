...
function problemA () {
  ...
  const itemOne = promisifiedReadFile('poem-one/stanza-01.txt')
  itemOne.then((stanza) => blue(stanza))
  console.log('stanza')
...
*/
  // promise version
  promisifiedReadFile('poem-one/stanzas-04.txt')
    .then((data) => {
      blue(data)      
...
*/
 ...
 /* 
  // promise version
  const promiseThre = promisifiedReadFile('poem-one/stanza-03.txt').then((data) => blue(data))
  const promiseFour = promisifiedReadFile('poem-one/stanza-04.txt').then((data) => blue(data))
  Promise.all([promiseThre, promiseFour]).catch((err) => {
    magenta(new Error(err))    
    console.log('done')
  })
*/
